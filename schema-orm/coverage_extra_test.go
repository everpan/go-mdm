...
func TestPK_isZero_VariousTypes(t *testing.T) {
	if !isZero(nil) { t.Fatalf("nil should be zero") }
	if !isZero("") { t.Fatalf("empty string zero") }
	if isZero("x") { t.Fatalf("non-empty string not zero") }
	if isZero(true) { t.Fatalf("true not zero") }
	if !isZero(false) { t.Fatalf("false is zero") }
	if isZero(1) { t.Fatalf("int non-zero") }
	if !isZero(0) { t.Fatalf("int zero") }
	if isZero(uint64(5)) { t.Fatalf("uint64 non-zero") }
	if !isZero(uint16(0)) { t.Fatalf("uint16 zero") }
	if isZero(float32(1.5)) { t.Fatalf("float32 non-zero") }
	if !isZero(float64(0)) { t.Fatalf("float64 zero") }
}

func TestTypes_SQLType2Type_AllCases(t *testing.T) {
	cases := []struct{ in SQLType; kind reflect.Kind }{
...
func TestTypes_Type2SQLType_MoreKinds(t *testing.T) {
	if Type2SQLType(reflect.TypeOf(true)).Name != "BOOLEAN" { t.Fatalf("bool mapping") }
	if Type2SQLType(reflect.TypeOf(float64(0))).Name != "FLOAT" { t.Fatalf("float mapping") }
	// default mapping for struct goes to TEXT in simplified impl
	type S struct{ A int }
	if Type2SQLType(reflect.TypeOf(S{})).Name != "TEXT" { t.Fatalf("default struct mapping") }
}

func TestTypes_IsBlob_IsTime_FalsePaths(t *testing.T) {
	st := SQLType{Name: "TEXT"}
	if st.IsBlob() { t.Fatalf("TEXT is not blob") }
	if st.IsTime() { t.Fatalf("TEXT is not time") }
}

func TestConvert_NilInputs(t *testing.T) {
	if ToXormColumn(nil) != nil { t.Fatalf("nil column") }
	if FromXormColumn(nil) != nil { t.Fatalf("nil column from") }
	if ToXormIndex(nil) != nil { t.Fatalf("nil index") }
	if FromXormIndex(nil) != nil { t.Fatalf("nil index from") }
	if ToXormTable(nil) != nil { t.Fatalf("nil table to") }
	if FromXormTable(nil) != nil { t.Fatalf("nil table from") }
}
...
func TestTable_GetColumn_Negative_and_UintPKPath(t *testing.T) {
	...
	tb := NewTable("t", typ)
	if tb.GetColumn("missing") != nil { t.Fatalf("expected nil get column") }
	if tb.GetColumnIdx("missing", 0) != nil { t.Fatalf("expected nil get column idx") }
	...
	if len(pk) != 1 || pk[0].(int64) != 42 { t.Fatalf("uint pk conversion failed: %v", pk) }
}

func TestIndex_XName_SchemaQualified(t *testing.T) {
	i := NewIndex("nm", UniqueType)
	name := i.XName("public.\"tbl\"")
	if name[:4] != "UQE_" { t.Fatalf("expected unique prefix for schema-qualified table") }
}
...
func TestConvert_Table_MapsAndIndexesFilled(t *testing.T) {
	 ...
	 tbl.AddIndex(i)
	 x := ToXormTable(tbl)
	 if x.Indexes["idx"] == nil { t.Fatalf("indexes not copied") }
	 if !x.Created["ID"] { t.Fatalf("created not copied") }
	 // back
	 b := FromXormTable(x)
	 if len(b.Columns()) != 1 || len(b.Indexes) != 1 || !b.Created["ID"] { t.Fatalf("round conversion failed") }
}

func TestColumn_ConvertID_InvalidNumber(t *testing.T) {
	c := &Column{SQLType: SQLType{Name: "INT"}}
	if _, err := c.ConvertID("not-a-number"); err == nil { t.Fatalf("expected parse error") }
}
...
func TestMarshal_Table_EmptyCollections(t *testing.T) {
	...
	tbl := NewEmptyTable()
	if tbl.Created == nil || tbl.Indexes == nil { t.Fatalf("zero table should have maps initialized") }
}

func TestConvertSQLType_Preservation(t *testing.T) {
	s := SQLType{Name: "DECIMAL", DefaultLength: 10, DefaultLength2: 2}
	x := ToXormSQLType(s)
	if x.DefaultLength != 10 || x.DefaultLength2 != 2 { t.Fatalf("to xorm preserve lengths") }
	back := FromXormSQLType(xs.SQLType{Name: s.Name, DefaultLength: 5, DefaultLength2: 1})
	if back.DefaultLength != 5 || back.DefaultLength2 != 1 { t.Fatalf("from xorm preserve lengths") }
}
